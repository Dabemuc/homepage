...
export const onRequestGet: PagesFunction<Env> = async (context) => {
  ...
  const rows = await db.select().from(schema.intro).all();
...
export const onRequestPut: PagesFunction<Env> = async (context) => {
  ...
  const existing = await db.select().from(schema.intro).where(eq(schema.intro.id, 1)).all();
  ...
  const updated = await db.select().from(schema.intro).where(eq(schema.intro.id, 1)).all();
